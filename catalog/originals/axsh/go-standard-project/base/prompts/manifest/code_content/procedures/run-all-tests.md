...
コンテナ環境が起動していることを確認する。未起動なら先にセットアップする。
...
> ワークフローファイルに記載されたカテゴリ名ではなく、常にこの動的発見の結果を使用すること。
...
- **`integration_test.sh`**: カテゴリ分割しても長時間かかる場合がある。
  - `WaitMsBeforeAsync` を十分大きく設定する（推奨: 300000ms 以上）。
  - 完了を待てない場合はバックグラウンド実行に切り替え、
    `command_status` ツールで定期的にポーリングする。
  - 出力ログの末尾を監視して進捗を確認する。
