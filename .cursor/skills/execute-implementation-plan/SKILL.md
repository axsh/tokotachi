...
2.  **ルールの読み込み**:
    *   `.cursor/rules/coding-rules.mdc` (コーディングルール)
    *   `.cursor/rules/testing-rules.mdc` (テスト実施ルール)
    *   `.cursor/rules/logging-rules.mdc` (ログ記述ルール)
...
3.  **コーディング**:
    ...
    *   `.cursor/rules/coding-rules.mdc` のスタイルや設計原則を厳守する。
    *   `.cursor/rules/logging-rules.mdc` のレベル基準に従い、DEBUG ログを積極的に挿入する。
4.  **こまめな Git コミット**:
...
テスト実施の詳細ルール（実行順序、修正ループ、タイムアウト方針等）は `.cursor/rules/testing-rules.mdc` を参照。
...
修正ループの詳細手順は `.cursor/rules/testing-rules.mdc` Section 3 を参照。
