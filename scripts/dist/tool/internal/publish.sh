...
# Usage: ./scripts/dist/tool/internal/publish.sh <tool-id> <version>
# Example: ./scripts/dist/tool/internal/publish.sh tt v1.0.0
...
if [[ ! -d "$RELEASE_DIR" ]] || [[ -z "$(ls -A "$RELEASE_DIR" 2>/dev/null)" ]]; then
  ...
  echo "  Run './scripts/dist/tool/internal/package.sh ${TOOL_ID} ${VERSION}' first."
  exit 1
fi
