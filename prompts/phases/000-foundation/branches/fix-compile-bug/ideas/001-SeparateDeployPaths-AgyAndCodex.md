...
| Target | 現状 | 変更後 |
|:---|:---|:---|
| **Antigravity** | `.agents/rules/`, `.agents/skills/` | `.agents/rules/`, `.agents/skills/`, `.agents/workflows/` |
...
- Antigravity は引き続き `.agents/` を使用するが、新たに `workflows/` ディレクトリを追加する

#### R2: Antigravity の capabilities を workflows にデプロイ

- `kind: capability` のエンティティは、Antigravity ターゲットにおいて `.agents/workflows/{id}/WORKFLOW.md` にデプロイすること（従来は `.agents/skills/{id}/SKILL.md`）
- `kind: procedure` のエンティティは引き続き `.agents/skills/{id}/SKILL.md` にデプロイする
- Branch Skills（far-knowledge 由来）は引き続き `.agents/skills/` にデプロイする
...
tmp/dist/
  antigravity/
    .agents/rules/      <- policies
    .agents/skills/     <- procedures, branch skills
...
- `antigravity.yaml`: `paths.workflows` を追加
...
graph TD
    ...
    subgraph "Deploy: Antigravity"
        AR[".agents/rules/"]
        AS[".agents/skills/"]
        AW[".agents/workflows/"]
    end

    subgraph "Deploy: Codex"
        CR[".codex/rules/"]
        CS[".codex/skills/"]
    end
...
1. `tt prompt compile --target antigravity` を実行し、`tmp/dist/antigravity/` 配下に以下が生成されることを確認:
   - `.agents/rules/` に policies が出力される
   - `.agents/skills/` に procedures と branch skills が出力される
   - `.agents/workflows/` に capabilities が出力される
   - `.agents/workflows/{id}/WORKFLOW.md` の形式であること
...
4. `tt prompt deploy --target antigravity` を実行し、プロジェクトルートの `.agents/` に正しくデプロイされることを確認:
   - `.agents/workflows/pre-push-knowledge-check/WORKFLOW.md` が存在すること
   - `.agents/skills/build-pipeline/SKILL.md` が存在すること（procedure 由来）
