...
- **Antigravity**: `rules/` と `skills/` に加え、`workflows/` の概念をサポートしている。Capabilities（再利用可能な能力）は skills に、Procedures（手順・ワークフロー）は workflows にデプロイすべきであり、全てを skills に統合すべきではない
...
2. **概念の不一致**: Antigravity の procedures は実質的に workflows (手順) だが、capabilities と一緒に skills にデプロイされているためユーザーにとって直感的でない
...
#### R2: Antigravity の procedures を workflows にデプロイ

- `kind: procedure` のエンティティは、Antigravity ターゲットにおいて `.agent/workflows/{id}.md` にデプロイすること（従来は `.agents/skills/{id}/SKILL.md`）。Antigravity の workflows はフラットな .md ファイルを配置する形式であり、サブディレクトリは使わない
- `kind: capability` のエンティティは `.agent/skills/{id}/SKILL.md` にデプロイする（従来と同じく skills だが、パスが `.agents/` から `.agent/` に変更）
...
tmp/dist/
  antigravity/
    ...
    .agent/skills/      <- capabilities, branch skills
...
- `Emit()` の procedures 出力先を `skillsDir` から `workflowsDir` に変更
- 出力ファイル形式を `{id}/SKILL.md`（フォルダ構造）から `{id}.md`（フラットファイル）に変更（procedures のみ）
- capabilities は引き続き `skillsDir` に `SKILL.md` として出力
...
- `{{procedure:id}}` の解決先を `workflows/` パスに変更（Antigravity ターゲットの場合）
...
1. `tt prompt compile --target antigravity` を実行し、`tmp/dist/antigravity/` 配下に以下が生成されることを確認:
   ...
   - `.agent/skills/` に capabilities と branch skills が出力される
   - `.agent/workflows/` に procedures が出力される
   - `.agent/workflows/{id}.md` のフラットファイル形式であること
...
4. `tt prompt deploy --target antigravity` を実行し、プロジェクトルートの `.agent/` に正しくデプロイされることを確認:
   - `.agent/skills/pre-push-knowledge-check/SKILL.md` が存在すること（capability 由来）
   - `.agent/workflows/build-pipeline.md` が存在すること（procedure 由来、フラットファイル）
...
| R2 (workflows デプロイ) | `antigravity_test.go` の `TestEmit_Antigravity` を更新し、procedures が `workflows/` に出力されることを検証 |
