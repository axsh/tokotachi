...
- 入力は以下の 2 経路をサポートする:
  - **JSON 経路**: `--file <path>` または `--stdin` で UTF-8 JSON ペイロードを受け取る
  - **CLI フラグ経路**: `--agent`, `--summary`, `--note` 等の個別フラグで直接指定する (内部で JSON ペイロードを構築)
- CLI フラグ一覧:

| フラグ | 必須 | 説明 |
|:---|:---|:---|
| `--file <path>` | 条件付き | JSON ファイルからペイロードを読む |
| `--stdin` | 条件付き | stdin から JSON ペイロードを読む |
...
- `--file`/`--stdin` と CLI フラグは排他とする (両方指定時はエラー)
- 入力スキーマに対するバリデーションを行い、違反時は明確なエラーコードで拒否する
- Git 情報の補完 (`--changed-paths-from-git` による収集、branch/head/merge_base の自動検出) は `tt` 本体が行う
- retry (exponential backoff, 最大 3 回) と `client_request_id` による冪等性は `tt` 本体が実装する
...
scripts/code/
  agent/
    notify.sh         # Coding Agent からのオプションを tt のオプションにマッピングする薄い shim
...
- `scripts/code/agent/notify.sh`: Coding Agent オプション → `tt agent notify` オプションのマッピング shim
...
| 良い raw note | `scripts/code/agent/notify.sh を notify の唯一の安定エントリポイントにする` | 対象と判断が明確 |
