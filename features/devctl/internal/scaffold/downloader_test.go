...
func TestGitHubDownloader_FetchFile(t *testing.T) {
	...
	content, err := d.FetchFile("catalog.yaml")
	require.NoError(t, err)
	assert.Equal(t, expectedContent, string(content))
}

func TestGitHubDownloader_FetchFile_NotFound(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	_, err := d.FetchFile("nonexistent.yaml")
	assert.Error(t, err)
}

func TestGitHubDownloader_FetchDirectory(t *testing.T) {
	fileContent := "# README\nHello"
	...
	files, err := d.FetchDirectory("templates/project-default/base")
	require.NoError(t, err)
...
func TestNewGitHubDownloader(t *testing.T) {
	tests := []struct {
		name    string
		url     string
		owner   string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			d, err := NewGitHubDownloader(tt.url)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
			require.NoError(t, err)
			assert.Equal(t, tt.owner, d.Owner)
			assert.Equal(t, tt.repo, d.Repo)
		})
	}
}
