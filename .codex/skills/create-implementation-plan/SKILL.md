...
2.  **ルールの読み込み**:
    *   `.codex/rules/planning-rules.md` を読み込みます。
3.  **ステータスの取得**:
...
6.  **テスト項目設計のセルフレビュー**:
    *   `.codex/rules/testing-rules.md` の §11 に従い、テスト項目がボトムアップ順序で設計され、観点チェックリスト (§11.3) が網羅されているか。
    *   §11.4 のセルフレビュー（網羅性・証拠の十分性・迂回排除・依存関係）の結果が記載されているか。
7.  **総合判定プロセスの計画**:
    *   `.codex/rules/testing-rules.md` の §12 に従い、全テスト完了後に総合判定を実施する手順が検証計画 (Verification Plan) に含まれているか。
8.  **E2Eテストコード化チェック**:
