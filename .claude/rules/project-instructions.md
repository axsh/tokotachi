...
`.claude/skills/` 配下に定義されたワークフローは、以下の順序で連携して動作します:

### 1. 仕様書作成フェーズ
**ワークフロー**: [`.claude/skills/create-specification/SKILL.md`](.claude/skills/create-specification/SKILL.md)
...
### 2. 実装計画作成フェーズ
**ワークフロー**: [`.claude/skills/create-implementation-plan/SKILL.md`](.claude/skills/create-implementation-plan/SKILL.md)
...
### 3. 実装実行フェーズ
**ワークフロー**: [`.claude/skills/execute-implementation-plan/SKILL.md`](.claude/skills/execute-implementation-plan/SKILL.md)

1. **人間**: 実装計画ファイルを指定
2. **AI**: `execute-implementation-plan.md` に従って実装を実行
   - 入力: `prompts/phases/000-xxx/branches/[ブランチ名]/plans/YYY-Name.md`
   - プロセス:
     - コーディングルール (`.claude/rules/coding-rules.md`) を遵守してコード実装
     - テストルール (`.claude/rules/testing-rules.md`) を遵守してテスト作成
     - 実装計画のチェックボックス `[ ]` → `[x]` で進捗管理
     - 進行中項目は `[/]` でマーク

### 4. ビルド・検証フェーズ
**ワークフロー**: [`.claude/skills/build-pipeline/SKILL.md`](.claude/skills/build-pipeline/SKILL.md)
...
### 調査ワークフロー（開発フローとは独立）
**ワークフロー**: [`.claude/skills/investigate/SKILL.md`](.claude/skills/investigate/SKILL.md)
