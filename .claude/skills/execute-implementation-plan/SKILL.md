...
2.  **ルールの読み込み**:
    *   `.claude/rules/coding-rules.md` (コーディングルール)
    *   `.claude/rules/testing-rules.md` (テスト実施ルール)
    *   `.claude/rules/logging-rules.md` (ログ記述ルール)
...
3.  **コーディング**:
    ...
    *   `.claude/rules/coding-rules.md` のスタイルや設計原則を厳守する。
    *   `.claude/rules/logging-rules.md` のレベル基準に従い、DEBUG ログを積極的に挿入する。
4.  **こまめな Git コミット**:
...
テスト実施の詳細ルール（実行順序、修正ループ、タイムアウト方針等）は `.claude/rules/testing-rules.md` を参照。
...
修正ループの詳細手順は `.claude/rules/testing-rules.md` Section 3 を参照。
