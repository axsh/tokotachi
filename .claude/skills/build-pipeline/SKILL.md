...
> `.claude/rules/testing-rules.md` を参照すること。
...
> **Linux / Remote-SSH**: `./scripts/process/build.sh --skip-etc` を使うこと（詳細は `.claude/rules/testing-rules.md` Section 1 参照）。
...
> **Linux / Remote-SSH**: `xvfb-run -a` でラップすること（詳細は `.claude/rules/testing-rules.md` Section 1 参照）。
...
テストが失敗した場合は、`.claude/rules/testing-rules.md` Section 3「エラー修正フロー」に従い修正する。
