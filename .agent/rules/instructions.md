...
## プロジェクト概要: vv3
本プロジェクトは、ワークフローAI化ツールの実装プロジェクトです。
...
1. **AI**: `scripts/process/build.sh` を実行
   - 全体ビルドと単体テストを実行
   - 失敗時は即座に修正して再実行
2. **AI**: `scripts/setup/setup_containers.sh` を実行（統合テスト用環境セットアップ）
3. **AI**: `scripts/process/integration_test.sh` を実行
   - 統合テストを実行
   - 失敗時は修正して該当テストのみ再実行 (`--specify` オプション使用)
4. **AI**: 必要に応じて全テストを再実行してリグレッション確認
