...
1.  **計画の読み込み**:
    ...
    *   **統合テスト計画の確認**: 実装計画の **Proposed Changes** に統合テストファイル（`tests/` 配下）の作成/修正が含まれているかを確認してください。含まれていない場合は、計画に不備がある可能性があります。その場合はユーザーに報告し、統合テストの追加要否を確認してください。
2.  **進捗の追跡**:
...
統合テストを実施する際は、以下を確認してください:

1.  **設定ファイルの配置**:
    *   テスト専用設定: `settings/test/config.yaml`
    *   シークレット: `settings/test/secrets/credentials.yaml` (Git管理外)
2.  **プロファイルの確認**:
    *   計画書に記載されたプロファイル名（例: `anthropic-test`, `gemini-test`）が設定ファイルに存在することを確認
3.  **API Keyの確認**:
    *   API Keyが設定されていない場合は `t.Fatalf()` でエラーとする（`t.Skip()` は使用しない）

### 3.3 修正と再テスト (Mandatory Fix Loop)
...
1.  **テスト失敗時の対応 (The Fix Loop)**:
    *   テストやビルドが失敗した場合は、以下のループを**成功するまで**繰り返します：
        ...
        3.  **Retry (失敗テストのみ)**: `--specify` で失敗テストのみを再実行して修正を確認する。
            - 例: `./scripts/process/integration_test.sh --categories xxx --specify "テスト名"`
        4.  **Resume (残りのテスト)**: 失敗テストが通過したら `--resume` で残りから再開する。
