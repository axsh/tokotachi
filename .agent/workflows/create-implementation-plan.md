...
> [!IMPORTANT]
> **統合テストの計画は必須です。** 新機能や変更に対応する統合テストファイルの作成/修正を必ず記述してください。
> 統合テストが不要な場合（例: 純粋な内部リファクタリングで外部挙動に変更がない場合）は、**不要である理由を明記**してください。

#### [NEW/MODIFY] [テストファイルパス](file://tests/[category]/[test_name]_test.go)
*   **Description**: [統合テストの概要]
*   **Category**: [テストカテゴリ名（integration_test.sh の --categories に対応）]
*   **Test Cases**:
    *   `[テストケース名]`: [テスト内容と期待される結果]

## Step-by-Step Implementation Guide

[ファイル単位ではなく、**時間軸**に沿った具体的な作業手順リスト]
[**重要**: 検証計画の「前」に実施手順を確定させること]
[**重要**: 統合テストファイルの作成/修正ステップを必ず含めること]
...
2.  **[Step Name]**:
    *   ...
N.  **統合テスト作成**:
    *   Create/Edit `tests/[category]/[test_name]_test.go`.
    *   [テストケースの具体的な記述指示]
...
2.  **Integration Tests**:
    ...
    ```bash
    ./scripts/process/integration_test.sh --categories "[Categories]" --specify "[Unique Test Case Name]"
    ```
...
4.  **テスト網羅性チェック (Platform Specific)**:
    *   (Go) 単体テストファイルが **Proposed Changes** に含まれているか。
    *   (Go) 統合テストファイルが **Proposed Changes** の **Integration Tests** セクションに含まれているか。含まれていない場合は、不要である理由を **User Review Required** に明記しているか。
    *   **Verification Plan** の統合テスト実行コマンド (`integration_test.sh`) が、Proposed Changes で作成/修正する統合テストファイルのカテゴリ・テスト名と整合しているか。
    *   TDDで計画されているか。
